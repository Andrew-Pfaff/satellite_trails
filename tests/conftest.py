...
def dummy_h5_file(tmp_path):
    ...
    source_mean = np.array([5.0, 20.0, 30.0], dtype=np.float32)
    source_std = np.array([5.0, 10.0, 10.0], dtype=np.float32)
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        f.create_dataset("source_mean", data=source_mean)
        f.create_dataset("source_std", data=source_std)
