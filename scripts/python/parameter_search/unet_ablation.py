"""Run baseline U-Net ablations from top Optuna trial rows."""
...
def evaluate_best_checkpoint(model, data_path, batch_size, num_workers):
    """
    Evaluates a trained U-Net checkpoint on the validation split.
    ...
    Returns:
        tuple: Best IoU threshold and its metric dictionary.
    """
...
def run_variant(trial_row, p_shift, use_batchnorm, args, output_dir):
    """
    Trains and evaluates one U-Net ablation variant.
    ...
    for trial_row in top_trials:
        for p_shift in SHIFT_VALUES:
            for use_batchnorm in (True, False):
                rows.append(run_variant(trial_row, p_shift, use_batchnorm, args, output_dir))
