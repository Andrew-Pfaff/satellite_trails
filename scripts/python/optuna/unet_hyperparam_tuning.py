...
from satellite_trail_segmentation.model.train import train_unet, create_cos_lr_sched
...
def create_objective(data_path, epochs):
    def objective(trial):
        ...
        scheduler = create_cos_lr_sched(optimizer, epochs, warmup_epochs=3)

        train_loss, val_loss, best_loss, total_epochs = train_unet(model, train_ds, val_ds, optimizer, scheduler, epochs, batch_size=64, num_workers=4, trial=trial)
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(study_name="unet_tuning", storage=f"sqlite:///{db_file_dir}/unet_optuna_study.db", 
                                load_if_exists=True, direction="minimize", pruner=optuna.pruners.MedianPruner(n_startup_trials=5, n_warmup_steps=8))
